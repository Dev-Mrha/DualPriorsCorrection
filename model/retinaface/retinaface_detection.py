...
from .utils_face.nms.py_cpu_nms import py_cpu_nms
import cv2
from .facemodels.retinaface import RetinaFace
from .utils_face.box_utils import decode, decode_landm
import time


class RetinaFaceDetection(object):
    def __init__(self, base_dir, network='mobilenet0.25_Final'): # [mobilenet0.25_Final, RetinaFace-R50]
        torch.set_grad_enabled(False)
        cudnn.benchmark = True
        self.pretrained_path = os.path.join(base_dir, 'pretrained_models', network+'.pth')
        ...
        self.load_model(not torch.cuda.is_available())
